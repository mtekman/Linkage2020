...
https://gitlab.com/mtekman/Linkage2020
...
***** Family 1183 (Canadian)
     ...
     #+CAPTION: Family 1183
     #+NAME: fig:1183
     [[./imgs/grr_screenshot_03_nors211641-family-1183.png]]
...
***** Family 1181 (British)
      ...
      #+CAPTION: Family 1181
      #+NAME: fig:1181
         [[./imgs/grr_screenshot_03_nors211641-family-1181.png]]
...
** Parse the Pedigree
   ...
   Here we will reuse some code from the [[pedfileCheck][above code]], and use the latest pedigree from =03_nors211641=.
...
** Remove inconsistent X-linked Markers from genotypes
   ...
   #+NAME: filterMarkers
   #+begin_src elisp :var olddir="/ssh:tetris@132.230.153.85:/home/tetris/work/jobs_for_people/robert/118_canspanbrit/02_second" :var newdir="/home/tetris/work/linkage/third" :var inconsis="/home/tetris/work/linkage/second/inconsistent.X.txt"
   ...
   #+begin_src bash :dir /ssh:tetris@132.230.153.85:/home/tetris/work/jobs_for_people/robert/118_canspanbrit/02_second
   ...
   #+begin_src bash :dir /home/tetris/work/linkage/third/
     wc -l genotypes maf.txt
   #+end_src
...
** Snpbutcher
   ...
   #+begin_src sh :session linkage :dir /ssh:tetris@132.230.153.85:/home/tetris/work/jobs_for_people/robert/118_canspanbrit/04_noinconsistentx/
   wc -l *
   #+end_src
   ...
   #+begin_src sh :session linkage :dir /ssh:tetris@132.230.153.85:/home/tetris/work/jobs_for_people/robert/118_canspanbrit/04_noinconsistentx/ :results verbatim
     snpbutcher2.py map.all.txt maf.txt genotypes -z -nh > map.txt
   #+end_src
...
** Run 4: Fingers crossed
   ...
   #+begin_src sh :session linkage
     cut -f 1,20-27 /mnt/robert/118_canspanbrit/04_noinconsistentx/genotypes | head -1
...
*** Debugging the methods again
    ...
    Let's add our new marker to our test genotypes
     ...
     #+begin_src sh :session linkage :results verbatim :file test.genos
...
**** Methods
     ...
     #+NAME: inconsistentXmarkers2
     #+begin_src elisp :var filename="test.genos"
       (cl-labels ((readtokens () (split-string (buffer-substring-no-properties
                                                 (point)
                                                 (progn (forward-line 1) (point)))
                                                "[	 ]" nil "[ \n]"))
     ...
     #+NAME: PrintOutInconsistentMarkers
     #+begin_src elisp :var outname=""
     ...
     #+CALL: PrintOutInconsistentMarkers(outname="~/work/linkage/third/inconsistent2.X.tsv")
     ...
     #+CALL: inconsistentXmarkers2(filename="test.genos")
...
*** Finding inconsistent X markers (updated)
   ...
   #+CALL: inconsistentXmarkers2(filename="/ssh:tetris@132.230.153.85:/home/tetris/work/jobs_for_people/robert/118_canspanbrit/02_second/genotypes") :results value file :file ~/work/linkage/third/inconsistent2.X.txt


     #+begin_src bash
       head -10 ~/work/linkage/third/inconsistent2.X.txt
     ...
     From 671 inconsistent markers, it's 2555... that's still a lot less than 8000, but still far far higher than I would have guessed. It sounds like one individual is messing up the linkage. I would guess it's the 57 to 96 relation. Let's see...
      ...
      #+CALL: relationships(filename=(expand-file-name "~/work/linkage/third/inconsistent2.X.txt"))

      #+NAME: Markers Removed By Relation
      #+Caption: Markers Removed By Relation
      | Occur | Relationship    |  ID1 |  ID2 |
      |-------+-----------------+------+------|
...
*** Filtering out inconsistent markers
   ...
   #+CALL: filterMarkers(olddir="/ssh:tetris@132.230.153.85:/home/tetris/work/jobs_for_people/robert/118_canspanbrit/02_second",newdir="/home/tetris/work/linkage/fourth",inconsis="/home/tetris/work/linkage/third/inconsistent2.X.txt")
   ...
   #+begin_src bash :dir /home/tetris/work/linkage/fourth/
     wc -l genotypes maf.txt
   #+end_src
...
folder and the =map.all.txt= file from the =02_analysis= folder.
   ...
   #+begin_src sh :dir /ssh:tetris@132.230.153.85:/home/tetris/work/jobs_for_people/robert/118_canspanbrit/05_noinconsistentX2
   wc -l *
   #+end_src
   ...
   #+begin_src sh :session new :dir /ssh:tetris@132.230.153.85:/home/tetris/work/jobs_for_people/robert/118_canspanbrit/05_noinconsistentX2 :results verbatim
     snpbutcher2.py map.all.txt maf.txt genotypes -z -nh > map.txt
   ...
   #+begin_src sh :session new :dir /ssh:tetris@132.230.153.85:/home/tetris/work/jobs_for_people/robert/118_canspanbrit/05_noinconsistentX2
...
*** Concatenate small map with full res X map
    ...
    #+begin_src sh :session new
      cat map.txt | grep -v "^X" > only.autosomes.half
...
* Thoughts so far
  ...
  Of the 5062 chromosome X markers in genotypes, only 412 chromosome X markers were deemed informative and consistent, in order to make it into the final analysis (91% of X markers were removed...)
    ...
    #+CAPTION: (Top) Allegro and (Bottom) GeneHunter for (Left) entire genome and (Right) only chromosome X.
    #+NAME: LinkageResults
    [[./imgs/summary.png]]

    #+CAPTION: Messner Output
...
** Why so few markers?
   ...
   The reasoning behind the removal of so many chromosome X markers are [[Linkage Analysis (Attempts 1-3)][outlined]] in [[Linkage Analysis (Attempt 4)][the]] previous [[Linkage Analysis (Attempt 5)][sections]], but to summarize:

    1. Allegro kept throwing "inconsistent marker" errors for the
       chromosome X markers.
    2. Examining [[rs5983047 Error on chrX][these]] markers [[Run 4: Fingers crossed][individually]] in the genotypes file
       showed these markers really *did* have inconsistent genotypes
       on the assumption that:

       1. No recombination occurs on the Y chromosome.
       2. Brothers must share one allele
       3. Fathers and Sons must share one allele

       By [[inconsistentXmarkers2][writing a parser]] which flagged the inconsistent markers which violated these
    ...
    3. To improve the number of chromosome X markers in the analysis, snpbutcher
    ...
    Due to the low resolution, gendercheck and GRR had to be disabled in this run for the analysis to run, otherwise it claimed that [[Gender error in 2024 (male by testing, female in ped)][individual 2024 was actually male]], and not female. This error never occured in any previous run with higher number of markers, so I assumed the stated gender was correct.

    The GRR in this report is actually from a previous run with a higher number of markers. I noticed that certain =father-son= relationships (yellow dots) [[GRR notes][clustered separately]] from other parental relationships (other yellow dots), but I assumed that this to be expected.


** Was it just one or two individuals messing up the analysis?

   I explored that idea in Table [[Markers Removed By Relation]] which shows the distribution of chromosome X markers removed based on the individuals who flagged the marker. Mostly it was:

   #+CAPTION: Number of X markers removed as flagged by specific relationships (here only father-son shown)
...
*** Number of Possible X Markers
     ...
     #+begin_src elisp :dir /ssh:tetris@132.230.153.85:/home/tetris/work/jobs_for_people/robert/118_canspanbrit/05_noinconsistentX2/ 
       (let ((uninf nil)
             (isX 0))
...
*** Process Files
   ...
   #+begin_src bash :dir /ssh:tetris@132.230.153.85:/home/tetris/work/jobs_for_people/robert/118_canspanbrit/ :results verbatim
     mkdir -p 06_fullX
   ...
   #+begin_src sh :dir /ssh:tetris@132.230.153.85:/home/tetris/work/jobs_for_people/robert/118_canspanbrit/06_fullX/ :results verbatim
     snpbutcher2.py map.all.txt maf.txt genotypes -u -z -nh > map.txt
   ...
   #+begin_src sh :dir /ssh:tetris@132.230.153.85:/home/tetris/work/jobs_for_people/robert/118_canspanbrit/06_fullX 
...
* Insights from Robert
  ...
  <2020-06-26 Fri 13:55>

  My filtering is *wrong*. My understanding was that the Y chromosome is semi-observable in linkage analysis by one of the X's that the males have. So one X would come from the mother, and one "X" would come from the father which would actually be a Y in disguise.

  This is *FALSE*. The Y chromosome never enters into the scope of linkage on the X chromosome, and we do not expect *anything* to be inherited between fathers and sons. It is therefore entirely possible for a father to have genotypes =AA= and a son to have =BB= because what we're actually seeing in the son is the allele from the mother simply doubled (so the mother would have given one =B= in this example).

  Given that a mother could have genotype =AB=, it is then completely valid for her to have two sons, where one is =AA= and the other is =BB=. In reality this would be =YA= and =YB= respectively, but we *do not observe the Y-chromosome* in linkage.

  So not only is my father-son filtering incorrect, but so is my brother-brother filtering. Yay.

  To quote Robert:

  #+begin_quote
  If you see Male-Male transmission in a pedigree, then that *excludes* X-linked inheritance.
  #+end_quote
  
  because males can only inherit from the mother as far as X-linked analysis goes. This seems obvious now in hindsight, and I feel I have finally understood this after 5 years.

** Mystery of Inconsistent Markers

  But then the mystery of why allegro throws an inconsistent marker error for the alleles I examined still remains. If I look at those [[rs211641 in the pedigree 1181][same]] two [[rs5983047 Error on chrX][markers]] I analysed they are actually consistent with the mothers alleles, so I am at a loss to what is going on.

  I now believe it is likely to do with how allegro is configured, since it is trying to interpret the X genotypes as autosomal (where you would indeed expect 1 allele to come from the father). I need to look into the (hard to find) manual.

** HaploForge Legitimacy

   There is also the question of whether haploforge resolves X-linked analyses wrong, since male-male transmission is not expected for X-linked analyses.

   By looking [[https://github.com/mtekman/HaploForge/blob/master/JS/resolvers/astar_handler.js#L89][here]] I can see that this is indeed how I interpreted how the inheritance works, but if I [[https://github.com/mtekman/HaploForge/blob/master/JS/resolvers/astar_handler.js#L99][look deeper]] into the code I can see that though a son will inherit the same colour allele as the father on one of their alleles, the actual haplotype checking is performed solely on the mother's haplotypes.

   So given that both father and son are hemizygous, what effectively happens here is that the mother and son are checked for haplotype consistency, but the /colour/ of one of the son's alleles comes from the father.

   It's not wrong, because the haplotype checking is correct, but the colouring would be misleading since it would indeed show male-male transmission and give the false impression that a Y chromosome is present. I should fix this.
...
* Linkage Analysis (Test 7)

  Due to some uncertainties in how the X-recessive and X-dominant penetrances are configured in the pipeline, Robert asked to do one analysis with individual =2033= changed to affected. 

  If it's X-recessive, mother's should not show the phenotype but son's always will, so it actually made no sense from the beginning that =2033= was unaffected. (Perhaps false parentage?)

  Let's copy some files over from run 5 (without the uninformative so we don't get that weird peak)

  #+begin_src sh :dir /ssh:tetris@132.230.153.85:/home/tetris/work/jobs_for_people/robert/118_canspanbrit/
   mkdir -p 07_affected2033
   cp 05_noinconsistentX2/{map.txt,maf.txt,genotypes,pedfile.pro} 07_affected2033/
  #+end_src

  #+RESULTS:

  Let's modify the pedfile to make 2033 affected

  #+begin_src text :tangle /ssh:tetris@132.230.153.85:/home/tetris/work/jobs_for_people/robert/118_canspanbrit/07_affected2033/pedfile.pro
    1181	7623	0	0	1	1
    1181	7624	0	0	2	1
    1181	32	7623	7624	2	1
    1181	22	7623	7624	2	1
...
** Result
   ...
   Wow. The LOD score jumped from 1.43 to 3.76  and the peak flattened (albeit still being quite wide).

   #+CAPTION: (Top) Allegro and (Bottom) GeneHunter for (Left) entire genome and (Right) only chromosome X.
   #+NAME: LinkageResults3
   [[./imgs/summary3.png]]
   ...
   | Program    | Chrom | Position            | Markers               |  LOD |
   |------------+-------+---------------------+-----------------------+------|
   | Allegro    | chrX  | 41235894 - 82959062 | rs787088 - rs7060133  | 3.76 |
   | Genehunter | chrX  | 35031054 - 67707597 | rs3124850 - rs5919577 | 3.69 |

   Included in the =07_affected2033= folder are the combined and individual linkage plots for this run (2033 is affected), and within the =05_noinconsistentX2= folder are the combined and individual linkage plots for the run with the same inputs as this one with the default pedigree (2033 is unaffected).

** Collect :noexport:

   #+begin_src sh :dir /ssh:tetris@132.230.153.85:/mnt/robert/118_canspanbrit/07_affected2033
     newdir=$(pwd)_collect
     mkdir -p $newdir
     cp README.txt $newdir/
     #
     # -- convert all ps files --
     postfiles=$(find ./ -type f -name "*.ps")
     for file in $postfiles; do
         newname=$(dirname "$file")/$(basename "$file" ".ps").pdf
         ! [ -e $newname ] && ps2pdf $file $newname
     done
     #
     function cp_pdf {
         from=$1
     ...
     # -- Allegro --
     # haplotypes + messner
     cp_pdf allegro $newdir/allegro/haplotypes
     cp allegro/messner_allegro_lod*.txt $newdir/allegro/
     # linkage plots
     cp_pdf allegro/plot_alohomora/ $newdir/allegro/plot_alohomora/
     cp_pdf allegro/plot_physical_adjusted/ $newdir/allegro/plot_physical_adjusted/
     families=$(echo allegro-*)
     for fold in $families; do
         cp_pdf $fold/plot_alohomora/ $newdir/allegro/$fold/plot_alohomora/
         cp_pdf $fold/plot_physical_adjusted/ $newdir/allegro/$fold/plot_physical_adjusted/
     done

     # -- Genehunter --
     # linkage plots + messner
     cp_pdf gh_200 $newdir/genehunter
     cp gh_200/messner_genehunter_lod*.txt $newdir/genehunter/
     familiesg=$(echo genehunter-family*)
     for fold in $familiesg; do
         cp_pdf $fold $newdir/genehunter/$fold/
     done

     # -- preprocessing --
     mkdir -p $newdir/preprocessing
     cp {gendercheck.txt,genotypes_Fam_*.pdf,merlin_lod.pdf,unlikely_genotypes.txt} $newdir/preprocessing

     # -- inputfiles --
     mkdir -p $newdir/inputfiles
     cp {genotypes,maf.txt,map.txt,pedfile.pro} $newdir/inputfiles

     # -- pedigrees --
     mkdir -p $newdir/pedigrees
     cp family_*.pdf $newdir/pedigrees
