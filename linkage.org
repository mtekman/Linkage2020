...
*** Finding inconsistent X markers (updated)
   ...
   #+RESULTS:
   [[file:~/work/linkage/third/inconsistent2.X.txt]]
...
** It could just be allegro being limited in this way. Maybe skip it and use genehunter for the earlier runs?


Allegro kept failing with inconsistent markers. I took this to mean that recombination was not expected on one of the male's alleles (Y-chromosome) and so I filtered out chrX markers for which no males shared an allele (e.g. brother-brother should share one allele, father-son should share one allele). This resulted in a loss of 2000 chrX markers, which after informative filtering in snpbutcher yielded only 412 markers....
