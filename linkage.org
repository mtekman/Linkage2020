        ...
        (let ((headers (-map #'string-to-number (cdr (readtokens))))
          ...
          (while (<= (length error-markers) 10)
            (let* ((linedata (readtokens))
                   ...
                   (genotypes (--map (if it (list (intern (car it)) (intern (cadr it))))
          ...
          (setq errmark error-markers))))
      
          ;;error-markers)))
  #+end_src

  #+begin_src elisp
    (cl-labels ((getrelationship (it)
                                 (car (plist-get it :relation)))
                (getids (it)
                        (sort (cdr (plist-get it :relation)) #'<))
                (elems-equal-p (first second)
                               (and (string= (getrelationship first)
                                             (getrelationship second))
                                    (equal (getids first)
                                           (getids second)))))
      (dolist (marker-err errmark)
        (let* ((marker (car marker-err))
               (data (cdr marker-err))
               (res nil))
          (dolist (relate-allele data)
            (if (>= (--count (elems-equal-p relate-allele it) data) 1)
                ;; If the element exists twice, nullify it
                (setf relate-allele nil))))))
  #+end_src
...
**** Real Genotypes
     ...
     Now let's try this on all our markers 
