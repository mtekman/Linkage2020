        ...
        (let ((headers (-map #'string-to-number (cdr (readtokens))))
          ...
          (while (not (eobp))
            (let* ((linedata (readtokens))
                   ...
                   (genotypes (--map (list (intern (car it))
                                           (intern (cadr it)))
                                     (--map (split-string it "" t) (cdr linedata))))
...
**** Test Genotypes
     ...
     Below is a small subset of the genotypes file containing two markers we know are inconsistent, which we will test on.
     ...
     Good, it found both markers and with valid reasons
